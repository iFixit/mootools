      ...
      var bound = function () {
         ...
         var result =
            !args && !length
               ...
               : self.apply(
                    context,
                    args && length ? args.concat(Array.slice(arguments)) : args || arguments
                 );
...
(function () {
   ...
   var parse = function (ua, platform) {
      ...
      platform = ua.match(/ip(?:ad|od|hone)/)
         ? 'ios'
         : (ua.match(/(?:webos|android)/) || ua.match(/mac|win|linux/) || ['other'])[0];
...
(function () {
   ...
   local.setDocument = function (document) {
      ...
      if (features.isHTMLDocument) {
         ...
         try {
            ...
            starSelectsClosed =
               selected && !!selected.length && selected[0].nodeName.charAt(0) == '/';
         } catch (e) {}
   ...
   local.search = function (context, expression, append, first) {
      ...
      this.push =
         !hasOthers && (first || (parsed.length == 1 && parsed.expressions[0].length == 1))
            ? this.pushArray
            : this.pushUID;
      ...
      ' ': function (node, tag, id, classes, attributes, pseudos, classList) {
         ...
         if (this.isHTMLDocument) {
            getById: if (id) {
               ...
               if (
                  (!item && node.all) ||
                  (this.idGetsName && item && item.getAttributeNode('id').nodeValue != id)
               ) {
   ...
   var pseudos = {
      ...
      empty: function (node) {
         ...
         return (
            !(child && child.nodeType == 1) && !(node.innerText || node.textContent || '').length
         );
      },
...
(function () {
   ...
   properties.text =
      document.createElement('div').textContent == null ? 'innerText' : 'textContent';
...
(function () {
   ...
   if (!window.addEventListener) {
      ...
      Element.Events.change = {
         base: function () {
            var type = this.type;
            return this.get('tag') == 'input' && (type == 'radio' || type == 'checkbox')
               ? 'propertychange'
               : 'change';
         },
         condition: function (event) {
...
(function () {
   ...
   Element.implement({
      getComputedStyle: function (property) {
         ...
         return computed
            ? computed.getPropertyValue(property == floatName ? 'float' : property.hyphenate())
            : '';
      },
   ...
   Element.implement({
      ...
      getOffsetParent: brokenOffsetParent
         ? function () {
              ...
              var isOffsetCheck =
                 styleString(element, 'position') == 'static' ? isOffsetStatic : isOffset;
